...
Original file is located at
    https://colab.research.google.com/drive/1xlqQQavGcnJn22qSLHjyd4vR70JcsCLT
"""
...
def ngram_count(google):
    alexa_match = alexa_counts * alexa_vc.transform([google]).T  # Woot vector multiply and transpose Woo Hoo!
    dict_match = dict_counts * dict_vc.transform([google]).T
    print = ('%s Alexa match:%d Dict match: %d') % (google, alexa_match, dict_match)
...
feature_names = ['length','entropy','alexa_grams','word_grams','diff','vowel-cons','digits']
...
radvizualizer = RadViz(classes=['legit','Dga'], features=feature_names)
...
df = allDomains
feature_matrix = df.drop(['class', 'label'], axis=1)
feature_matrix.head()

target = df.drop(['length', 'entropy', 'alexa_grams',	'word_grams',	'diff',	'vowel-cons',	'digits'], axis=1)
target.head()
...
test_feature = feature_matrix[2:2]
test_target = target[2:2]
...
test_data = pd.read_csv('/home/logs/current/dns.log', names=['domain'])
